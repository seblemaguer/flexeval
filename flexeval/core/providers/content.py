...
class AssetsProvider(Provider):
    ...
    def local_url(self, path: str, _from: Optional[str] = None) -> str:
        ...
        path : str
            The path of the assets
        _from : Optional[str]
            the source of the assets (None means it is provided by the core of FlexEval!)
        ...
        if _from is None:
            ...
            return self._url_prefix + "/flexeval" + path
        elif _from[:3] == "mod":
            name_mod = _from[4:]
            return self._url_prefix + "/flexeval/mods/" + name_mod + path
        else:
        ...
        try:
            try:
                return send_from_directory(
                    current_app.config["FLEXEVAL_INSTANCE_DIR"] + "/assets/" + "/".join(repositories),
                    file,
                )
            except Exception:
                if repositories[0] == "flexeval":
                    try:
                        return send_from_directory(
                            current_app.config["FLEXEVAL_DIR"] + "/assets/" + "/".join(repositories[1:]),
                            file,
                        )
                    except Exception:
...
class TemplateProvider(Provider):
    ...
    def __init__(self, folder: str):
        ...
        try:
            ...
            raise TemplateImportError("Import from flexeval's templates failed.")

        self._logger.info(" loaded and bound to  " + folder)

    def register(self, name_module: str):
        """Register a module by automatically list its templates
        ...
        ------
        TemplateImportError
            If something is wrong with the template files
        """
        try:
            self._register_module(name_module)
        except Exception:
            raise TemplateImportError("Import from mod:" + name_module + " failed.")
    ...
    def _register_flexeval(self):
        """Register the flexeval templates"""
    ...
    def register_instance(self):
        ...
        self._logger.debug(
            ...
            % (current_app.config["FLEXEVAL_INSTANCE_DIR"] + "/templates", self._folder)
        )
    ...
    def _register_module(self, name: str):
        ...
        self._logger.debug(
            ...
            % (current_app.config["FLEXEVAL_DIR"] + "/mods/" + name + "/templates", self._folder)
        )
        copytree(
